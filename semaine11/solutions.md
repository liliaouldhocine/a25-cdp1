...
## **Exercice 2 : Calculatrice Scientifique avec Historique**
...
class Calculatrice:
    ...
    def addition(self, a, b):
        ...
        try:
            ...
            raise ValueError("Les deux valeurs doivent être des nombres")
    ...
    def division(self, a, b):
        ...
        try:
            ...
            if b == 0:
                operation = f"ERREUR: {a} ÷ {b} -> Division par zéro"
                self.historique.append(operation)
                raise DivisionZeroError("Division par zéro impossible")

            resultat = a / b
            operation = f"{a} ÷ {b} = {resultat:.4f}"
            self.historique.append(operation)
            return resultat
    ...
    def racine_carree(self, a):
        ...
        try:
            ...
            if a < 0:
                operation = f"ERREUR: √{a} -> Nombre négatif"
                self.historique.append(operation)
                raise RacineNegativeError("La racine carrée d'un nombre négatif n'est pas définie")

            resultat = math.sqrt(a)
            operation = f"√{a} = {resultat:.4f}"
            self.historique.append(operation)
            return resultat
    ...
    def effacer_historique(self):
        ...
        confirmation = input("Êtes-vous sûr de vouloir effacer l'historique? (o/n): ")
        if confirmation.lower() == 'o':
...
def interface_calculatrice():
    ...
    operations = {
        '1': ('Addition', calc.addition, 2),
        '2': ('Soustraction', calc.soustraction, 2),
        '3': ('Multiplication', calc.multiplication, 2),
        '4': ('Division', calc.division, 2),
        '5': ('Puissance', calc.puissance, 2),
        '6': ('Racine carrée', calc.racine_carree, 1)
    }

    while True:
        print("\n=== CALCULATRICE SCIENTIFIQUE ===")
        for key, (nom, _, _) in operations.items():
            print(f"{key}. {nom}")
        ...
        if choix == '9':
            ...
            nom_operation, fonction, nb_nombres = operations[choix]
            ...
            except RacineNegativeError as e:
                print(f"Erreur mathématique: {e}")
            except Exception as e:
                print(f"Erreur inattendue: {type(e).__name__}: {e}")
